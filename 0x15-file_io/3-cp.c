...
int main(int ac, char **av)
{
	...
	file1 = open(av[1], O_RDONLY); /* Read only from file1 to return number of bytes */
	if (file1 == -1) 
		dprintf(STDERR_FILENO, "Error: Can't read from file %s\n", av[1]), exit(98);

	/* octal (0), rw- (6), rw- (6), r-- (4) */
	file2 = open(av[2], O_WRONLY | O_CREAT | O_TRUNC, 0664); /* Write only in file2 */ 
	if (file2 == -1) 
	...
	/* Copy data from file1 to file2 */
		...
		writeBytes = write(file2, buff, readBytes); /* Write data from file1 to file2 */
		if (writeBytes == -1)
			dprintf(STDERR_FILENO, "Error: Can't write to %s\n", av[2]), exit(99);
